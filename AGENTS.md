...
- Designer Print crop: **Save detected fiducial** overwrites the project default logo (`fiducial.png` / `logo.png`) with the patch inside the green box on the prepared scan; the app does not back up the previous file. The match score can be high on the wrong spot, so the user can drag the green box on the right-hand scan (inside moves, edge scales one axis, corner keeps aspect). All field outlines follow as one group. Save is enabled after a hand move, or a match ≥ 0.7, and only while the box is inside the print crop. Clicks outside the box still place registration marks.
...
- Indexer: **QC** menu puts batch actions on the top level. **QC Document** (`Validate document`, `Review document comments`) is a submenu below those. **QC → Batch QC Complete** shows the same Yes/No "Batch completed" dialog as the last page of the last document. Yes calls `_complete_current_batch` (same folder move). No batch loaded: information dialog, no move. Pending CSV writes are flushed before the dialog.
...
- Indexer: **Page → Reorder pages** opens a maximised two-pane window (autofit scans, no sizing controls) to fix multipage scan order in the current document file (PDF or TIFF). Left panel: last page already in correct order. Right panel: the page that should follow it. **Move page** inserts the right-hand page immediately after the left; changes are written to the document on disk and caches are refreshed. Navigating the left panel resynchronises both panels to the same page; the right panel may then be advanced only to the same page or any later page. **Close** returns to indexing on the left panel's page.
- Indexer: clicking a text field on the page (including Integer, Decimal, Date, and other text subtypes) places the caret in the value box under the close-up so typing can start immediately. The small text box under the field stays visible and in sync, and does not take keyboard focus. Enter moves to the next text field on the page and returns the caret to that value box.
