...
- Indexer: **QC → Batch QC Complete** shows the same Yes/No "Batch completed" dialog as the last page of the last document. Yes calls `_complete_current_batch` (same folder move). No batch loaded: information dialog, no move. Pending CSV writes are flushed before the dialog.
