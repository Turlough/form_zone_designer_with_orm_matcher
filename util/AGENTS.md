...
- Indexer page-visit field nudge: `util/field_group_align.py` (`FieldGroupAlign`, `placed_rect`, `drag_group_bounds`, `hit_group_bounds`). Maps logo-relative outlines into prepared-canvas pixels. Edge drag scales one axis; corner drag keeps aspect ratio; interior drag translates. The align's logo is frozen so a later fiducial result does not shift the group again. Not persisted.
