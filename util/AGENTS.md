...
- Designer print crop: `util/print_crop.py` (`parse_print_crop`, `prepare_scan_page`, `canvas_to_crop_uv` / `crop_uv_to_canvas`) plus `load_print_crop` / `save_print_crop` in `designer_persistence.py`. Optional `print_crop` `{x, y, width, height}` in `json/project_config.json` is one template-pixel rectangle for all pages (finished-page / crop-mark area). Missing key: Indexer resizes the whole scan to the template page size. When set: resize the scan to the crop size and paste it at `(x, y)` on a white template-sized canvas, then run fiducial matching. Field Review thumbnails use the same prepare step.
