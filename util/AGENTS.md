...
- Designer print crop: `util/print_crop.py` (`parse_print_crop`, `prepare_scan_page`, `crop_prepared_page_for_display`, `display_origin`, `canvas_to_crop_uv` / `crop_uv_to_canvas`, `fiducial_bbox_inside_crop`) plus `load_print_crop` / `save_print_crop` in `designer_persistence.py`. Optional `print_crop` `{x, y, width, height}` in `json/project_config.json` is one template-pixel rectangle for all pages (finished-page / crop-mark area). Missing key: Indexer resizes the whole scan to the template page size. When set: resize the scan to the crop size and paste it at `(x, y)` on a white template-sized canvas, then run fiducial matching. Field Review thumbnails use the same prepare step. Indexer centre panel and field close-up crop the prepared canvas back to the pasted scan (`crop_prepared_page_for_display`) so white margins are not shown.
- Designer Basic Indexing Config: `load_project_config` / `save_indexing_config` / `indexing_config_from_project` in `designer_persistence.py` merge `project_name`, `batch_folder`, `import_filename`, `lookup_list`, `lookup_prime_index`, `pages_without_fiducial`, `all_uppercase` into `json/project_config.json` (blank `lookup_list` removes the key; missing `all_uppercase` is False)
...
- Indexer OCR: gated by `INDEXING_ASSISTANT_ENABLED` in `.env` (default off; see `util/indexing_assistant_config.py`); `util/gemini_ocr_client.py` (`GOOGLE_API_KEY` or `GEMINI_API_KEY` when enabled); text normalization in `util/ocr_text_utils.py` (`all_uppercase` from project config, default False)
...
- Document page reorder (Indexer **Page → Reorder pages**): `util/document_reorder.py` — `move_page_after` rewrites the current PDF (PyMuPDF) or multipage TIFF on disk via a temp file in the same folder
...
- Fiducials: `util/fiducial_paths.py` — default logo candidates and per-page `logo-pN.png` (1-based N) overriding default when present. `save_detected_fiducial` overwrites the default file (`find_default_logo`, else `fiducial.png`) with a prepared-scan crop at a `(top_left, bottom_right)` box (Print crop **Save detected fiducial**: matcher result or the green box after the user drags it)
- Indexer page-visit field nudge: `util/field_group_align.py` (`FieldGroupAlign`, `placed_rect`, `drag_group_bounds`, `hit_group_bounds`); also used by the Designer Print crop draggable fiducial box. Maps logo-relative outlines into prepared-canvas pixels. Edge drag scales one axis; corner drag keeps aspect ratio; interior drag translates. The align's logo is frozen so a later fiducial result does not shift the group again. Not persisted.
