...
- Indexer **QC → Batch QC Complete**: same Yes/No "Batch completed" dialog as the last page of the last document (`_confirm_and_complete_batch`). Yes advances the batch via `_complete_current_batch`. No batch open: information dialog, no move.
...
- Indexer text entry: clicking a `TextField` (including Integer, Decimal, Date, and other text subtypes) on the page shows `IndexTextDialog` under the field without taking keyboard focus. The caret goes to `IndexDetailPanel.value_text_edit` (value box under the close-up), with the current value selected, so typing can start immediately. Enter moves to the next text field on the page and returns the caret to that box. The page image and its scroll area do not take click focus.
