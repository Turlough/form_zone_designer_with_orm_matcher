...
7. If the **green box** is on the wrong spot (the match score can be high even when it is wrong), drag it onto the real fiducial on the right-hand scan. All field zones move with it. Drag inside the box to move it, an edge to stretch one direction, or a corner to resize while keeping its shape. Adjust until the field zones sit on the printed boxes. Clicking outside the green box still places registration marks.
8. When the green box sits on the mark, click **Save detected fiducial**. That overwrites the default file in `fiducials/` (`fiducial.png` or `logo.png`) with the patch inside the green box. Confirm first — the app does not keep a backup. Then page through again; other pages that use the default fiducial now match this scanned appearance. If you had to resize the box to make fields line up, the print crop is off: fix the crop on the left instead, because Indexer does not rescale fields. Moving the crop or changing page puts the green box back where the matcher found it.
9. **Save print crop** writes `print_crop` to `json/project_config.json`. **Clear** removes the key so Indexer stretches scans to the full template again.
